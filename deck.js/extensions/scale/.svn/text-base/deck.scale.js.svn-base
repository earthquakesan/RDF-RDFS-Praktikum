	...
	*/
	...
	$d.bind('deck.init', function() {
		...
		$d.unbind('keydown.deckscale').bind('keydown.deckscale', function(e) {
			...
			if (e.which === opts.keys.frame || $.inArray(e.which, opts.keys.frame) > -1) {
				if($('#slide-area').hasClass('container'))
					$('#slide-area').removeClass('container');
				else
					$('#slide-area').addClass('container');
			}
		});
